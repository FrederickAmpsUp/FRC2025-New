...
public:
    ...
    void update_control(const Nav& nav) {
        ...
        //this->m_algaeIntakeActuator.Set(ctre::phoenix::motorcontrol::ControlMode::Position, (nav.get_desired_algae_intake_angle() / (2.0*M_PI) + c_algaeIntakeActuatorOffset) * 4096);
        ...
        if (this->m_capstanHomingStep == CapstanHomingStep::STOPPED) {
            ...
            if (this->m_capstanIterationsStopped > 10) {
                this->m_capstanIterationsStopped = 0;
            ...
            auto capCtr = ctre::phoenix6::controls::PositionVoltage(units::angle::turn_t(glm::mix(this->m_capstanMinPos, this->m_capstanMaxPos, nav.get_desired_capstan_angle() * 3.0f)));
            this->m_capstan.SetControl(capCtr);
        }
    }
private:
